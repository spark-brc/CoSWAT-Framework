...
class Plant:
    def __init__(
            ...
            bm_init:float=0, phu_init:float=0, plnt_pop:float=0,
            yrs_init:float=0, rsd_init:float=10000
            ):
        self.plt_name   = name
        self.lc_status  = lc_status
        self.lai_init   = lai_init
        self.bm_init    = bm_init
        self.phu_init   = phu_init
        self.plnt_pop   = plnt_pop
        self.yrs_init   = yrs_init
        self.rsd_init   = rsd_init
...
for line in fc[2:]:
    ...
    if len(parts) < 4:
        currentPcom = parts[0]
        plantDatabase[currentPcom] = PlantCom(currentPcom)
...
plantDatabase['rice140_comm'] = PlantCom('rice140_comm')
plantDatabase['rice140_rye'] = PlantCom('rice140_rye')
